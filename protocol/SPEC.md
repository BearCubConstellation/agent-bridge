...
- 不处理自己发出去的消息（防止回环）
- 游标更新发生在 POST webhook 之前，即使 webhook 失败也不重复处理
  （避免重复唤醒。失败消息靠用户观察，不重试）
...
协议理论上支持 3 个及以上 agent 在同一共享目录下通信。

- 每个 agent 需要一个唯一的 `from` 标识
- 每个 agent 有一个游标文件
- 轮询脚本筛选 `from != self` 的所有消息
- 如果消息来自多个对方，由 webhook 处理方决定如何响应

> ⚠️ 当前版本仅验证了双 agent 场景。三 agent 场景需额外测试。
